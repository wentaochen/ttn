...
func TestScheduleSync(t *testing.T) {
	...
	s.Sync(1<<32 - 10)
	a.So(s.timestamp, ShouldEqual, 1<<32-10)
	a.So(s.getFullTimestamp(10), ShouldEqual, 1<<32+10)

	s.Sync(1000)
	a.So(s.timestamp, ShouldEqual, 1<<32+1000)
}
...
func TestScheduleGetOption(t *testing.T) {
	...
	s := NewSchedule(GetLogger(t, "TestScheduleGetOption"))
...
func TestScheduleSchedule(t *testing.T) {
	a := New(t)
	s := NewSchedule(GetLogger(t, "TestScheduleSchedule"))
	...
	_, score, _ := s.GetOption(50000, 100000)
	a.So(score, ShouldEqual, 100)
